...
import StatisticsScreen from '../emprendedor/Statistics.jsx';
...
import MisProyectos from '../emprendedor/MisProyectos.jsx';
import ChatEmprendedor from '../chat/ChatEmprendedor.jsx';
import ChatInversionista from '../chat/ChatInversionista.jsx';
import EntrepreneurFoundyCard from '../emprendedor/FoundyCard.jsx';
function App() {
  ...
  if (usuarioLogueado) {
    if (pantallaLogueado === 'settings') {
      return (
        <PerfilConfiguracion
          ...
          onOpenProjects={() => setPantallaLogueado('projects')}
    ...
    if (pantallaLogueado === 'create-project' && !esInversionista) {
      return <CrearProyecto nombreUsuario={usuarioLogueado.usuario} usuarioData={usuarioLogueado} proyectoInicial={proyectoEnEdicion ? { ...proyectoEnEdicion, monto: proyectoEnEdicion.monto_objetivo || '', retorno: proyectoEnEdicion.retorno || '' } : undefined} proyectoId={proyectoEnEdicion?.id_proyecto} onCerrarSesion={handleCerrarSesion} onBackHome={irAHome} onOpenProjects={() => { setProyectoEnEdicion(null); setPantallaLogueado('projects'); }} onOpenChat={() => setPantallaLogueado('chat')} onOpenSettings={irAConfiguracion} onOpenStatistics={() => setPantallaLogueado('statistics')} onOpenFoundyCard={() => setPantallaLogueado('foundy-card')} />;
    }

    if (pantallaLogueado === 'projects' && !esInversionista) {
      return <MisProyectos usuarioData={usuarioLogueado} onCerrarSesion={handleCerrarSesion} onBackHome={irAHome} onOpenCreateProject={() => { setProyectoEnEdicion(null); setPantallaLogueado('create-project'); }} onOpenProjects={() => setPantallaLogueado('projects')} onOpenStatistics={() => setPantallaLogueado('statistics')} onOpenSettings={irAConfiguracion} onOpenChat={() => setPantallaLogueado('chat')} onOpenFoundyCard={() => setPantallaLogueado('foundy-card')} onEditProject={(project) => { setProyectoEnEdicion(project); setPantallaLogueado('create-project'); }} />;
    }

    if (pantallaLogueado === 'chat') {
      const ChatPage = usuarioLogueado.tipo_usuario === 'Inversionista' ? ChatInversionista : ChatEmprendedor;
      return <ChatPage usuarioData={usuarioLogueado} onBackHome={irAHome} onOpenProjects={() => setPantallaLogueado('projects')} onCerrarSesion={handleCerrarSesion} onOpenCreateProject={() => { setProyectoEnEdicion(null); setPantallaLogueado('create-project'); }} onOpenSettings={irAConfiguracion} onOpenStatistics={() => setPantallaLogueado('statistics')} onOpenFoundyCard={() => setPantallaLogueado('foundy-card')} />;
    }

    if (pantallaLogueado === 'statistics' && !esInversionista) {
      return <StatisticsScreen usuarioData={usuarioLogueado} onCerrarSesion={handleCerrarSesion} onBackHome={irAHome} onOpenCreateProject={() => setPantallaLogueado('create-project')} onOpenProjects={() => setPantallaLogueado('projects')} onOpenStatistics={() => setPantallaLogueado('statistics')} onOpenSettings={irAConfiguracion} onOpenChat={() => setPantallaLogueado('chat')} onOpenFoundyCard={() => setPantallaLogueado('foundy-card')} />;
    }
    ...
    if (pantallaLogueado === 'foundy-card') {
      if (!esInversionista) return <EntrepreneurFoundyCard usuarioData={usuarioLogueado} onLogout={handleCerrarSesion} onBackHome={irAHome} onOpenCreateProject={() => setPantallaLogueado('create-project')} onOpenProjects={() => setPantallaLogueado('projects')} onOpenStatistics={() => setPantallaLogueado('statistics')} onOpenSettings={irAConfiguracion} onOpenChat={() => setPantallaLogueado('chat')} onOpenFoundyCard={() => setPantallaLogueado('foundy-card')} />;
