...
function Register({ onSwitchToLogin }) {
    ...
    const handleRegister = async (e) => {
        ...
        if (!/^[A-Za-zÁÉÍÓÚáéíóúÑñ ]{2,50}$/.test(formData.nombre)) {
            setErrorMsg('First name must be between 2 and 50 letters.');
            setLoading(false);
            return;
        }

        if (!/^[A-Za-zÁÉÍÓÚáéíóúÑñ ]{2,50}$/.test(formData.apellidos)) {
            setErrorMsg('Last name must be between 2 and 50 letters.');
            setLoading(false);
            return;
        }

        if (!/^[A-Za-z0-9_]{4,20}$/.test(formData.usuario)) {
            setErrorMsg('Username must be 4 to 20 characters: letters, numbers, or _.');
            setLoading(false);
            return;
        }
        ...
        if (!formData.tipo_usuario) {
            setErrorMsg('Select a user type.');
            setLoading(false);
            return;
        }

        if (!/^\d{8}$/.test(formData.telefono)) {
            setErrorMsg('Phone number must contain exactly 8 digits.');
            setLoading(false);
            return;
        }

        if (!/^\d{8}-\d$/.test(formData.dui)) {
            setErrorMsg('DUI must use the format 12345678-9.');
            setLoading(false);
            return;
        }

        if (formData.contrasena.length < 8 || !/(?=.*[a-z])(?=.*[A-Z])(?=.*\d)/.test(formData.contrasena)) {
            setErrorMsg('Password must have 8 characters, an uppercase letter, a lowercase letter, and a number.');
            setLoading(false);
            return;
        }
        ...
        try {
            ...
            setErrorMsg(error.message || 'The account could not be saved. Check your Supabase connection.');
        } finally {
            setLoading(false);
        }
    };
        ...
        <main className="min-h-screen bg-[#f4faf8] text-[#142d39] lg:grid lg:grid-cols-[minmax(31rem,0.9fr)_1.1fr]">
            <section className="relative overflow-hidden px-6 py-7 sm:px-10 lg:flex lg:min-h-screen lg:flex-col lg:px-[clamp(2.5rem,7vw,7rem)] lg:py-10">
                ...
                <div className="relative z-10 mx-auto mt-14 w-full max-w-xl lg:my-auto lg:py-12">
                    <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#079184]">Your next step</p>
                    <h1 className="mt-3 max-w-lg text-4xl font-bold leading-[1.05] tracking-tight text-[#113b47] sm:text-5xl">Supporting Salvadoran entrepreneurship.</h1>
                    <p className="mt-5 max-w-md text-sm leading-6 text-[#5d7277]">Create your profile and connect with opportunities that can take your idea further.</p>
                    ...
                    <form className="mt-8" onSubmit={handleRegister} noValidate>
                        <div className="grid gap-x-5 gap-y-5 sm:grid-cols-2">
                            <label className="text-xs font-semibold text-[#31515a]">First name<input className={fieldClass} type="text" name="nombre" minLength={2} maxLength={50} pattern="[A-Za-zÁÉÍÓÚáéíóúÑñ ]+" placeholder="Juan" value={formData.nombre} onChange={handleChange} required /></label>
                            <label className="text-xs font-semibold text-[#31515a]">Last name<input className={fieldClass} type="text" name="apellidos" minLength={2} maxLength={50} pattern="[A-Za-zÁÉÍÓÚáéíóúÑñ ]+" placeholder="Pérez García" value={formData.apellidos} onChange={handleChange} required /></label>
                            ...
                            <label className="text-xs font-semibold text-[#31515a]">User type<select className={fieldClass} name="tipo_usuario" value={formData.tipo_usuario} onChange={handleChange} required><option value="">Select an option</option><option value="Emprendedor">Entrepreneur</option><option value="Inversionista">Investor</option></select></label>
                            <label className="text-xs font-semibold text-[#31515a]">Phone<input className={fieldClass} type="tel" name="telefono" placeholder="77777777" value={formData.telefono} inputMode="numeric" maxLength={8} pattern="[0-9]{8}" onChange={handleChange} required /></label>
                            ...
                            <label className="text-xs font-semibold text-[#31515a]">Password<input className={fieldClass} type="password" name="contrasena" minLength={8} maxLength={72} pattern="(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9]).{8,}" placeholder="••••••••" value={formData.contrasena} onChange={handleChange} required /></label>
                            <label className="text-xs font-semibold text-[#31515a]">Confirm password<input className={fieldClass} type="password" name="confirmarContrasena" minLength={8} maxLength={72} placeholder="••••••••" value={formData.confirmarContrasena} onChange={handleChange} required /></label>
                        </div>
            ...
            <section className="relative hidden min-h-screen overflow-hidden bg-[#075d65] lg:block" aria-label="Emprendimiento y comercio local">
                ...
                <div className="absolute inset-x-12 bottom-14 max-w-lg text-white">
                    ...
                    <p className="text-sm font-semibold uppercase tracking-[0.2em] text-[#b8f0df]">Ideas find momentum</p>
                    <h2 className="mt-4 text-5xl font-bold leading-[1.02] tracking-tight">Your idea deserves a community.</h2>
                    <p className="mt-5 max-w-sm text-base leading-7 text-white/80">Foundy connects people who imagine the future with those who can help build it.</p>
                </div>
            </section>
        </main>
    );
