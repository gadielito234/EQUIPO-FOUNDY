...
function Register({ onSwitchToLogin }) {
    ...
    return (
        <main className="grid min-h-screen grid-cols-1 bg-white text-[#11133f] min-[701px]:grid-cols-[38%_62%]">
            <section className="min-h-auto overflow-y-auto bg-[#05656c] px-6 py-8 min-[701px]:min-h-screen min-[701px]:px-[38px] min-[701px]:pt-[38px] min-[701px]:pb-7">
                <img className="mx-auto mb-7 block w-[126px] max-w-[54%] min-[701px]:mb-[34px]" src="/images/foundy-logo.png" alt="Foundy" />
                <div className="mx-auto w-full max-w-[430px]">
                    ...
                    <form onSubmit={handleRegister}>
                        <div className="grid grid-cols-1 gap-x-4 gap-y-3.5 min-[701px]:grid-cols-2">
                            ...
                            <label className="block text-[10px] leading-tight text-[#f6ffff]">DUI<input className="mt-1.5 block h-[34px] w-full rounded bg-white px-[11px] text-[12px] text-[#1c2449] outline-none placeholder:text-[#a0a5ad] focus:ring-[3px] focus:ring-white/30" type="text" name="dui" placeholder="23560189-0" value={formData.dui || ''} onChange={handleChange} required /></label>
                            <label className="block text-[10px] leading-tight text-[#f6ffff]">Contraseña<input className="mt-1.5 block h-[34px] w-full rounded bg-white px-[11px] text-[12px] text-[#1c2449] outline-none placeholder:text-[#a0a5ad] focus:ring-[3px] focus:ring-white/30" type="password" name="contrasena" placeholder="••••••••" value={formData.contrasena} onChange={handleChange} required /></label>
                            <label className="block text-[10px] leading-tight text-[#f6ffff]">Confirmar contraseña<input className="mt-1.5 block h-[34px] w-full rounded bg-white px-[11px] text-[12px] text-[#1c2449] outline-none placeholder:text-[#a0a5ad] focus:ring-[3px] focus:ring-white/30" type="password" name="confirmarContrasena" placeholder="••••••••" value={formData.confirmarContrasena} onChange={handleChange} required /></label>
                        </div>
                        <button className="mx-auto mt-[25px] block h-[34px] w-32 rounded bg-[#197e8c] text-[12px] text-white transition hover:-translate-y-px hover:bg-[#2698a5] disabled:cursor-not-allowed disabled:opacity-65 disabled:transform-none" type="submit" disabled={loading}>{loading ? 'Registrando...' : 'Register'}</button>
                    </form>
                    <p className="mt-[18px] text-center text-[11px] text-white/80">¿Ya tienes cuenta? <button className="p-0 font-bold text-white hover:underline" type="button" onClick={onSwitchToLogin}>Inicia sesión</button></p>
                </div>
            </section>
            <section className="relative min-h-[380px] bg-[url('/images/emprendedores-negocios.jpg')] bg-cover bg-center min-[701px]:min-h-screen" aria-label="Emprendimiento y comercio local">
                <div className="absolute left-1/2 top-1/2 w-[78%] -translate-x-1/2 -translate-y-1/2 rounded-[9px] bg-white px-6 py-[26px] text-center text-[24px] font-extrabold leading-[1.22] text-[#11133f] shadow-[0_12px_24px_rgba(17,19,63,0.12)] min-[701px]:w-[56%] min-[701px]:max-w-[420px] min-[701px]:text-[clamp(22px,2.1vw,34px)]">Today a small<br />investment can<br />become a great<br />future</div>
            </section>
        </main>
    );
}
