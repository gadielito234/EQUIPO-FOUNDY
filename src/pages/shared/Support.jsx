      ...
      <div className="mx-auto max-w-6xl px-5 py-7 sm:px-8 lg:px-12">
        <header className="mt-2 rounded-[28px] bg-[#006b73] px-6 py-8 text-white shadow-[0_16px_35px_rgba(17,52,60,0.12)] sm:px-8 sm:py-10">
          <div className="max-w-2xl">
            <p className="text-[10px] font-bold uppercase tracking-[.2em] text-[#bde3dc]">{t('supportCenter')}</p>
            <h1 className="mt-3 text-3xl font-bold tracking-tight sm:text-4xl">{t('howCanWeHelp')}</h1>
            <p className="mt-3 max-w-xl text-sm leading-6 text-[#d9efeb]">{t('supportIntro')}</p>
          </div>
        </header>
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[1.08fr_.92fr]">
          <section className="rounded-[24px] border border-[#dfe5e5] bg-white p-5 shadow-[0_10px_30px_rgba(17,52,60,0.04)] sm:p-6" aria-labelledby="faq-heading">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 id="faq-heading" className="text-lg font-semibold text-[#1d3f42]">{t('faq')}</h2>
                <p className="mt-1 text-xs text-[#687577]">{t('quickAnswers')}</p>
              </div>
            </div>
          ...
          <section className="rounded-[24px] border border-[#dfe5e5] bg-white p-5 shadow-[0_10px_30px_rgba(17,52,60,0.04)] sm:p-6" aria-labelledby="contact-heading">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 id="contact-heading" className="text-lg font-semibold text-[#1d3f42]">{t('contactSupport')}</h2>
                <p className="mt-1 text-xs text-[#687577]">{language === 'es' ? 'Cuéntanos qué necesitas y te responderemos.' : 'Tell us what you need and we will get back to you.'}</p>
              </div>
            </div>
            ...
            <form className="mt-6 space-y-4" onSubmit={handleSubmit}>
              <label className="block text-xs font-semibold text-[#526164]">
                ...
                <input required value={form.subject} onChange={(event) => updateForm('subject', event.target.value)} className="mt-2 w-full rounded-xl border border-[#ccd8d5] bg-[#fbfcfb] px-3 py-2.5 text-sm font-normal text-[#29494c] outline-none transition focus:border-[#006b73] focus:ring-2 focus:ring-[#006b73]/10" placeholder={language === 'es' ? '¿Con qué necesitas ayuda?' : 'What do you need help with?'} />
              </label>
              <label className="block text-xs font-semibold text-[#526164]">
                {language === 'es' ? 'Mensaje' : 'Message'}
                <textarea required value={form.message} onChange={(event) => updateForm('message', event.target.value)} className="mt-2 min-h-32 w-full resize-y rounded-xl border border-[#ccd8d5] bg-[#fbfcfb] px-3 py-2.5 text-sm font-normal text-[#29494c] outline-none transition focus:border-[#006b73] focus:ring-2 focus:ring-[#006b73]/10" placeholder={language === 'es' ? 'Describe tu pregunta o problema...' : 'Describe your question or issue...'} />
              </label>
            ...
            {sent && <p role="status" className="mt-4 flex items-center gap-2 rounded-xl bg-[#eaf8f1] px-3 py-2.5 text-xs font-semibold text-[#1b7f61]"><CheckCircle2 size={15} /> {language === 'es' ? 'Hemos recibido tu solicitud.' : 'Your request has been received.'}</p>}

            <div className="mt-6 border-t border-[#e5e9e8] pt-5">
              <p className="text-xs font-semibold text-[#526164]">{language === 'es' ? '¿Prefieres el correo electrónico?' : 'Prefer email?'}</p>
              ...
              {onOpenChat && <button type="button" onClick={onOpenChat} className="mt-4 block text-xs font-semibold text-[#1b7f61] hover:underline">{language === 'es' ? 'Chatear con soporte' : 'Chat with support team'}</button>}
            </div>
          </section>
        </div>
      </div>
